...
# Add parent dir so we can import cricket_api
...
def _load_teams() -> dict:
    """Load local team fan data (rivalries, fun facts, fan base info)."""
    with open(os.path.join(DATA_DIR, "teams.json"), "r", encoding="utf-8") as f:
        return json.load(f)


# ── Live & Current Match Tools ────────────────────────────────

def get_live_matches() -> dict:
    """Get all currently live and recently completed cricket matches with real-time scores.

    Fetches data from CricAPI's currentMatches endpoint. Returns live scores,
    match status, venue, teams, and score details for all ongoing matches.

    Returns:
        dict: Live/current matches with real-time scores, team names, venue, status.
              Includes 'data' array with match objects.
    """
...
def get_live_cricket_scores() -> dict:
    ...
    Returns quick score summaries for every match currently in progress.

    Returns:
        dict: Array of live score objects with team names, scores, status, match type.
    """
...
def get_match_details(match_id: str) -> dict:
    ...
    Includes scorecard, teams, venue, toss, result, man of the match, and score breakdowns.

    Args:
        match_id: The CricAPI match UUID (e.g., '2d448290-d882-4e67-9a4a-f62dfea9a51a').
...
def get_ipl_matches() -> dict:
    ...
    Searches for IPL series and returns all matches in the latest IPL season
    with real-time scores and results.

    Returns:
        dict: IPL series info with all matches, scores, schedule, and results.
    """
    # First find the IPL series
    ...
    # Get the latest IPL series (first result, sorted by recency)
    latest_ipl = series_list[0]
    series_id = latest_ipl.get("id", "")

    # Fetch full series info with all matches
...
def get_team_info(team_code: str) -> dict:
    ...
    This uses curated local fan data enriched with team culture and community info.
...
def get_player_details(player_name: str) -> dict:
    ...
    Fetches real player data from CricAPI including batting/bowling style,
    date of birth, country, and role.
...
def get_matches_for_team(team_name: str) -> dict:
    ...
    Searches through current matches to find ones where the given team is playing.
    ...
    """
    # Resolve team code to full name if needed
